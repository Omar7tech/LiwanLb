...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "inset-0 hidden flex-1 flex-row items-center justify-center space-x-1 text-lg  text-[#3A3B3A] lg:flex",
        className
      )}
    >
      {items.map((item, idx) => (
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0 }}
          className={cn(
            "absolute inset-x-0 top-16 z-50 flex w-full flex-col gap-4 rounded-sm bg-[#fafafa] px-4 py-8 shadow-sm",
            className
          )}
        >
          {children}
