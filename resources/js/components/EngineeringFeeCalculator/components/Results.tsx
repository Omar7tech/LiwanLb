...
const Results: React.FC<ResultsProps> = ({
  ...
  // Format large numbers with abbreviations to prevent overflow
  // Only abbreviate numbers larger than 1 million
  const formatLargeNumber = (num: number): string => {
    ...
    // Only use abbreviations for numbers greater than 1 million
    if (absNum >= 1e30) {
      ...
      // For numbers 1 million and below, use normal locale formatting
  ...
  const generateAIAnalysis = async () => {
    ...
    const prompt = language === 'ar'
        ...
        - التكلفة المقدرة: $${formatLargeNumber(estimatedCost)}
        - تكلفة الأساس لكل متر مربع: $${formatLargeNumber(baseCostPerSqm)}
        ...
        - Estimated Cost: $${formatLargeNumber(estimatedCost)}
        - Base Cost per Square Meter: $${formatLargeNumber(baseCostPerSqm)}
  ...
  const handleUserMessage = async () => {
    ...
    try {
      ...
      const conversationPrompt = language === 'ar'
        ...
        - التكلفة الإجمالية: $${formatLargeNumber(estimatedCost)}
        - تكلفة المتر المربع: $${formatLargeNumber(baseCostPerSqm)}
        ...
        - Total cost: $${formatLargeNumber(estimatedCost)}
        - Cost per square meter: $${formatLargeNumber(baseCostPerSqm)}
      ...
      const hasProjectData = analysisText.includes('$' + formatLargeNumber(estimatedCost)) ||
        analysisText.includes(formatLargeNumber(baseCostPerSqm));
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-3 sm:gap-4 lg:gap-6 lg:max-w-2xl lg:mx-auto">
        <div className="group hidden lg:block p-6 bg-white rounded-lg shadow-sm border border-gray-100 transition-all duration-300 hover:shadow-lg hover:border-[#F2AE1D]/30 hover:-translate-y-1">
          ...
          <p className="text-2xl font-semibold text-[#3a3b3a] font-cairo group-hover:scale-105 transition-transform duration-300">
            ${formatLargeNumber(baseCostPerSqm)}/m²
          </p>
        </div>
        ...
        <div className="group hidden lg:block p-6 bg-white rounded-lg shadow-sm border border-gray-100 transition-all duration-300 hover:shadow-lg hover:border-green-200 hover:-translate-y-1">
          ...
          <p className="text-3xl font-bold text-green-700 font-cairo group-hover:scale-105 transition-transform duration-300">
            ${formatLargeNumber(estimatedCost)}
          </p>
        </div>
