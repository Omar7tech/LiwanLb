  ...
  const generateAIAnalysis = async () => {
    ...
    try {
      const prompt = language === 'ar'
        ? `قم بتحليل نتائج حساب الأتعاب الهندسية التالية وقدم رؤى احترافية مع إحصائيات دقيقة:
        
        بيانات المشروع:
        - التكلفة المقدرة: $${estimatedCost.toLocaleString()}
        - نسبة الأتعاب المطبقة: ${applicablePercentage.toFixed(1)}%
        - الحد الأدنى للأتعاب: $${minimumFee.toLocaleString()}
        - تكلفة الأساس لكل متر مربع: $${baseCostPerSqm.toLocaleString()}
        - نسبة الأتعاب إلى التكلفة الإجمالية: ${((minimumFee / estimatedCost) * 100).toFixed(1)}%
        - إجمالي الاستثمار: $${(estimatedCost + minimumFee).toLocaleString()}
        ...
        قدم تحليلاً احترافياً (100-150 كلمة) يتضمن:
        1. إحصائيات دقيقة ومقاييس الأداء
        ...
        : `Analyze the following engineering fee calculation results and provide professional insights with solid statistics:
        
        Project Data:
        - Estimated Cost: $${estimatedCost.toLocaleString()}
        - Applied Fee Percentage: ${applicablePercentage.toFixed(1)}%
        - Minimum Fee: $${minimumFee.toLocaleString()}
        - Base Cost per Square Meter: $${baseCostPerSqm.toLocaleString()}
        - Fee to Cost Ratio: ${((minimumFee / estimatedCost) * 100).toFixed(1)}%
        - Total Investment: $${(estimatedCost + minimumFee).toLocaleString()}
        ...
        Provide a professional analysis (100-150 words) including:
        1. Solid statistics and performance metrics
      ...
      const errorPrompt = language === 'ar'
        ? `حدث خطأ فني. يرجى تقديم تحليل أساسي للبيانات التالية:
        ...
        قدم تحليلاً معمارياً أساسياً  بناءً على هذه البيانات.`
        : `A technical error occurred. Please provide basic analysis of the following data:
        ...
        Provide basic architectural analysis based on this data.`;
  ...
  const handleUserMessage = async () => {
    ...
    try {
      ...
      const conversationPrompt = language === 'ar'
        ? `أنت خبير معماري ذكي من Liwan Architecture. استخدم بيانات المشروع لإعطاء إجابات دقيقة ومفيدة.
        
        بيانات المشروع الحالية:
        - التكلفة الإجمالية: $${estimatedCost.toLocaleString()}
        - نسبة الأتعاب: ${applicablePercentage.toFixed(1)}%
        - الحد الأدنى للأتعاب: $${minimumFee.toLocaleString()}
        - تكلفة المتر المربع: $${baseCostPerSqm.toLocaleString()}
        - نسبة الأتعاب للتكلفة: ${((minimumFee / estimatedCost) * 100).toFixed(1)}%
        ...
        كن خبيراً حقيقياً:
        1. استخدم بيانات المشروع في إجاباتك
        ...
        6. إذا السؤال غير معماري، قل "اسألني عن التصميم أو التكاليف أو البناء"`
        : `You are a smart architectural expert from Liwan Architecture. Use project data to give accurate, helpful answers.
        
        Current project data:
        - Total cost: $${estimatedCost.toLocaleString()}
        - Fee percentage: ${applicablePercentage.toFixed(1)}%
        - Minimum fee: $${minimumFee.toLocaleString()}
        ...
        Be a real expert:
        1. Use project data in your answers
        ...
        6. If not architectural, say "Ask me about design, costs, or construction"`;
      ...
      const hasProjectData = analysisText.includes('$' + estimatedCost.toLocaleString()) ||
        analysisText.includes(applicablePercentage.toFixed(1) + '%') ||
  ...
  return (
    <div id="results-section" className="mt-12  animate-fadeIn">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div className="group hidden lg:block p-6 bg-white rounded-lg shadow-sm border border-gray-100 transition-all duration-300 hover:shadow-lg hover:border-yellow-300 hover:-translate-y-1">
          <p className="text-sm font-medium text-gray-700 mb-3 font-instrument-sans group-hover:text-yellow-600 transition-colors">
            {t('applicableFee')}
          </p>
          <p className="text-2xl font-semibold text-yellow-600 font-instrument-sans group-hover:scale-105 transition-transform duration-300">
        ...
        <div className="lg:hidden p-6 bg-white rounded-lg shadow-sm border border-gray-100 animate-scaleIn" style={{ animationDelay: '0.3s' }}>
          <p className="text-sm font-medium text-gray-700 mb-3 font-instrument-sans">
            {t('applicableFee')}
          </p>
        ...
        <div className="lg:hidden p-6 bg-white rounded-lg shadow-sm border border-gray-100 animate-scaleIn" style={{ animationDelay: '0.4s' }}>
          <p className="text-sm font-medium text-gray-700 mb-3 font-instrument-sans">
            {t('minimumFee')}
          </p>
          <p className="text-3xl font-bold text-green-700 font-instrument-sans">
                ...
                {isConversationMode && (
                  <div className="border-t border-gray-100 pt-3 sm:pt-4">
                    <div className="relative group">
                      <div className="absolute inset-0 bg-gradient-to-r from-[#F2AE1D]/5 to-[#3a3b3a]/5 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                      <textarea
                        value={userInput}
